...
Rewritten 2026-09-16 as a design, not yet built; the paragraphs marked *today* describe the code as it stands.

*Today:* a war is a flag between two peoples set by a coin flip at first contact. Each tick each side rolls a strike at a fixed rate and a win takes a random enemy world; when the enemy is down to its home one more win ends everything. Distance plays no part, colonies are almost never glassed, and nobody fights twice. Enslavement, vassalage, revolt, uplift and breeding stay as they are below.
...
**The decision layer** is one appraisal of a fight against a target that every choice routes through: effective strength (Military at home minus expeditions away, plus stationed relief, plus allies who would actually join, minus the enemy's other wars, dark age or plague), terrain (home bonus, defence grid, lag: a war prosecuted only by a four-thousand-year crossing is discounted steeply), fog (what intelligence says, widened by its age), and odds as a probability. Posture sets the bar: opportunists above three in four, conquerors above two in five with a small chance honour compels them lower, the vengeful above three in ten against the grudge target and nobody else, xenophobes above a third against the different. Nobody launches an expedition the appraisal says cannot take its first world; a conqueror that scouted and stayed home gets a line. The same appraisal decides peace (when the odds of taking more fall below the cost), pacts, relief, betrayal and scouting, and steers research before any war: a people that sees a neighbour it cannot beat tilts toward weapons and defence, seeks a pact if confederate, bends the knee early if submissive.
...
**Expected numbers**: a war of five to thirty thousand years, one to five worlds changing hands, ending in peace about six times in ten, capitulation two, extinction one; a third of pairs fighting again.

**Build order**: war object and front; postures, will and the appraisal with intelligence; parasites as a war mode; expeditions and scouts; pacts, honour and reputation; the reference batch rerun at each step.
...
- Build the war redesign in its five steps. Parasites are starved until then: Infection is faced about three times per world because contact is rare; contact from holdings and infection as a war mode are the fix.
