...
**Proposed next (2026-09-16), not built: parasites, viruses and memetic viruses as homeless fleets.** A parasite would hold no worlds. It would be a set of swarms, each an expedition with a strength and a base on a world its host still owns. A swarm at a base is at war with the base's owner over the worlds within a hop; when it wins a world the world stays the host's in name but is ridden, and the world's share of the host's Military becomes a new swarm there (the host carries it as strength away, until the world is cleaned). Swarms above a size split, sending half to a world within a hop; swarms at one base merge; a swarm spawns scouts at a level each. A swarm below half a level dies; a host burning a ridden world kills the swarm there and glasses the world. Research is pooled across swarms from their total strength and half the tree of every people ridden. The parasite is extinct when no swarm remains and master of every people it has fully ridden. At birth the first swarm rides the beasts of the cradle at strength one, grows slowly, and can drift as spores to a world within a few light years at three hundred years a light year until a host people comes within a hop. Memetic beacons could later join the same model. What it replaces: the parasite Civ's worlds and the infection war mode built today; what it keeps: the species, its tree, its legends. Open choices: whether the cradle counts as a base without ownership (proposed yes), and whether a ridden people's own wars carry the swarm along (proposed yes: a ridden fleet is a swarm in transit).
...
- Parasites, viruses and memetic viruses as homeless fleets (see the proposal under "War, submission, enslavement"). Then proactive scouting by posture, and tuning of pact acceptance and relief, which are rare because contact is rare.
