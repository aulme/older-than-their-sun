...
type Tale struct {
	...
	Blamed  int  // at myth, whom the tale now blames instead, or -1
...
func (w *World) readTestament(c *Civ, l *Legacy) {
	...
	for _, td := range l.Testament {
		...
		if own {
			if restored := w.restore(c, f.ID); restored {
				n++
			}
			continue
		}
...
// restore gives a people back a tale it had forgotten, or one it never
// held of its own past, from its own relic. Returns whether anything came back.
func (w *World) restore(c *Civ, fact int) bool {
	for _, t := range c.Lore {
		if t.Fact != fact {
			continue
		}
...
func (w *World) wearStep(c *Civ, t *Tale, f *Fact) {
	...
	if blame {
		if e := w.foe(c); e >= 0 && e != f.Subject {
			...
			c.Tally.Blamed++
			if w.R.Float64() < 0.3 {
				w.log("The %s now tell that it was the %s who %s. It was not.", c.Name, w.Civs[e].Name, w.deedOf(f))
			}
			return
		}
	}
...
// foe is the people this one holds the most against, alive or dead, or -1.
