...
func (w *World) garrison(c *Civ) {
	...
	c.Tally.Garrisons++
	w.log("The %s send %s to hold %s.", c.Name, shipsWord(g.Ships), w.star(to.Star))
}
...
func (w *World) muster(c, e *Civ, world int, cause string, n int) {
	...
	w.log("The %s gather their ships at %s.", c.Name, w.star(star))
}
...
func (w *World) musterStep(c *Civ) {
	...
	case ch.StandDown:
		c.Muster = nil
		if w.Cfg.TraceAI {
			w.log("[the %s stand down at %s]", c.Name, w.star(m.Star))
		}
	}
}
