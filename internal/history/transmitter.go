...
func (w *World) seeded(c *Civ, l *Legacy) {
	...
	w.log("The %s hear the transmitter at %s. Something comes down the signal with it and begins to move through their minds. They call it %s.", c.Name, w.star(l.Star), p.Name)
}
...
func init() {
	def(&Filter{
		...
		Overcome: func(w *World, c *Civ) {
			w.log("The %s hear the transmitter at %s, and do not answer, and forbid anyone to listen again.", c.Name, w.star(w.transmitter.Star))
		},
		Scar: func(w *World, c *Civ) {
			...
			c.Morale -= 1
			l.Listeners++
			w.log("Some of the %s hear the transmitter at %s and are changed. A cult of the signal grows among them and is never quite rooted out.", c.Name, w.star(l.Star))
			w.corrupted(c, l)
		},
		Decline: func(w *World, c *Civ) {
			...
			if w.R.Float64() < 0.3 {
				...
				w.log("From %s a new signal goes out, in the voice of the %s.", w.star(home), c.Name)
				return
			}
