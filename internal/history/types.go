...
type Civ struct {
	...
	Rival        int             // the menacing neighbour it builds its fleets against and watches hardest, or -1; see watchRival
	ClientCalls  int             // its clients' calls it has heard; see client.go
	ClientCame   int             // and come to, joining or sending ships: its record, read as its guarantee
	Rate         float64         // as a vassal, the share of its whole income it pays its patron; see tribute.go
	PaidShort    int             // ticks it paid its tribute short since the bond's last review
	RateTarget   float64         // the rate its patron's character and the bond's making set, before noise
	RateReviewed Year            // when the bond was last reviewed
	PaysFirst    bool            // it pays its tribute before its own works, fleets and road
	...
	Worsted      map[int]int     // the wars it came off worst in against each, remembered: see mind.Bar
