...
func (w *World) fathomed(c, e *Civ, how string) {
	...
	w.told(FFathomed, c, e, -1).with(P{"how": how, "mutual": rec.Mutual, "kin": w.kin(c, e), "since": w.Now - since, "wars": c.Fought[e.ID]})
	if rec.Mutual {
		w.openPair(c, e)
	}
}
...
func (w *World) broker(z, c, e *Civ) {
	z.Tally.Brokered++
	w.fact(FBrokered, z, c, -1).with(P{"to": e.ID})
