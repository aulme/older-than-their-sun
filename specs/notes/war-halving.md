...
## Addendum, step 19 (2026-09-21): the loops cut, the count reread, the case closed as a measurement

**(a) Wars with the loop pairs removed** (a loop pair is a pair with more than twenty wars between them in one world), the four batches since step 15:

| batch | wars | loop pairs | wars in loops | wars outside loops | distinct pairs at war | first wars |
|---|---|---|---|---|---|---|
| step 15 (`d6a9bfc`) | 10198 | 32 | 3978 | 6220 | 4321 | 3664 |
| step 16 (`1e913f6`) | 4370 | 17 | 1428 | 2942 | 1964 | 1692 |
| step 17 (`b657c23`) | 4862 | 7 | 1375 | 3487 | 2309 | 1948 |
| step 18 (`f723969`) | 11989 | 71 | 9366 | 2623 | 1509 | 1280 |

So the halving at step 16 is not the loops alone: outside them the count still fell from 622 to 294 a world, and the distinct pairs at war from 432 to 196. But the per-seed spread is the larger fact. First wars by seed at step 15: 511, 812, 173, 208, 126, 50, 127, 210, 408, 1039; at step 16: 120, 167, 296, 494, 94, 56, 31, 313, 49, 72. Twenty-fold between seeds of one batch, and four seeds of step 15 (1, 2, 9, 10) hold three quarters of its first wars. A seed 12 at 200 stars run on both builds with `-ai` gave 29 declarations on the step-15 build and 80 on the current one. Ten seeds cannot resolve a factor of two when one seed can be a factor of twenty; the batch's war count measures which seeds rolled a cascade (a line of sunderings, each heir at war with each other and every open war continued against every heir) and which rolled a loop.

**(b) Why the loop did not end.** Every loop pair of consequence was two pacifist heirs of one sundering (54 of the 71 loop pairs at step 18, with causes "a border", "the sundering" and "the old quarrel", the first being the council's fall-through cause when no posture names one). The claim rule (`mind.Bar`) wanted a claimed world's holder at the vengeful bar whatever the posture, so a pacifist that would declare on nobody declared on its sibling; its will of 0.3 was spent in two thousand years, the war ended in peace with nothing taken, the truce of five to fifteen thousand years ran out, the claim still stood (claims clear only when an heir holds the whole realm or the sundering wears to myth, and a people's own heavy myths hardly wear), and it declared again. Fixed at step 19: a pacifist is a pacifist before its claim (`Bar` returns nothing for a pacifist before it reads `Claim`); the batch after the change is in the step-19 ledger.

**Closed as a measurement problem.** The remaining leads (the grudge writes, the opportunity bar, the expansion multiplier, the wall, the tithe) were not run to ground; the peoples-per-world fall at step 16 (464 → 383) is the one systematic difference and would account for a third of the pairs by itself if pairs scale with the square of the peoples standing. For the next round, read wars as **first wars per distinct pair** and as **wars per thousand people-ticks**, on twenty seeds, before calling any change a halving; `techstats` should print both.
