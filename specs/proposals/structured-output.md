...
**Status:** In Progress (2026-09-22). Stages 1 (events), 2 (lookups) and 3 (the writer and the view) implemented; stage 4 (the codex) as `specs/plan.md` orders it (step 15, after every proposal that changes what the fiction is). Independent of `decline.md` and `galaxy-in-motion.md`; **follows `names.md` stage 1**, whose record shape this proposal serialises (the adjustments are marked "names:" below).
...
**Stage 3: the writer and the view.** The output directory, and the view as its first consumer. **Done 2026-09-22.**
...
- *As built*: `internal/record` holds the types of the four files and reads and writes a run directory (`record.Write`, `record.Load`); `FORMAT.md` sits beside them and is copied in with `data/` and `codex/` (an index alone, until stage 4), and `format_test.go` fails on a field the writer emits that the document does not name. The history exports itself (`history/export.go`: the dossier, the state with every people's knowledge, the chronicle with its parameters passed through JSON so a record built in memory is what a reader of the files sees, the tellings with the believed year, the teller's sort and weight and the rank among its dearest), and `internal/writer` adds the names table, the voices and the code revision. `worldgen -out DIR` (default `out/<seed>`) writes; `-legends` renders the directory just written and `-read DIR` one written earlier; `-until Y` stops the age at Y and marks the dossier truncated. `internal/legends` reads the run and nothing else: `lines.go`, `describe.go` and `telling.go` moved out of the history with the text tables they need read from `data/` (`tables.go`), the galaxy rebuilt from the stars and the bloods from the species records, the names resolved by a `names.Table` built from the rows; the legends of ten seeds at 200 stars, seed 3 in full and seed 1 with the debug lines are byte-identical to the reference. `techstats` reads the same records, from the seeds it runs or from directories (`-runs`), through a small `legends.Reader` for the words it prints. A testament is no longer text: `Inscription` keeps the tale, the maker's judgment and a `Frozen` record of what the telling read of the world then (which parties it could hold in mind, had met, were rising and living; whether the star was its own), and the view tells it with those. **The fold test** (`writer/fold_test.go`) folds the chronicle on four seeds, one cut short, and reproduces the durable fields listed in its table: a star's holder and life; a people's blood, cradle, seat, stage, fate and cause, worlds, nodes, scars, boons, miracles, line, sleep, master, word and infections and wars; a blood's powers; a remain's kind, maker, star, state, condition, finder and listeners; a war's sides, cause and ending; a plague's hosts. It forced twenty-nine **silent kinds** (`civ_born` to `power_held` in `events.json`, marked `silent`): the setters that make each durable change write them (`history/notes.go`), so the two cannot drift; they have no line and no weight, and take their ids after every told event so the ids the tellings hash stay put, which is what keeps the legends byte-identical. Choices made without asking: the record's numbers are kept as JSON literals so an integer count and a fraction render as they did; a term or a morality in a parameter is written as the record's own shape; `plague.hosts` is folded as a count over the peoples still rising rather than carried; a star's class and a people's traits are snapshot (the first class is the galaxy's, and a drift is a `drifted` fact the fold does not read); the `-ai` reason text names terms and remains by key, since the sim no longer holds the view's words; `TestSameHistory` digests the told events only.
